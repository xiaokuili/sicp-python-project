...
def tweet_location(tweet):
    ...
    "*** YOUR CODE HERE ***"
    p = make_position(tweet.get("lat"), tweet.get("lon"))
    return p
...
def make_tweet_fn(text, time, lat, lon):
    ...
    def f(attr):
        return  {'text': text, 'time': time, 'lat': lat, 'lon': lon}.get(attr)
    return f
...
def analyze_tweet_sentiment(tweet):
    ...
    for word in extract_words(tweet_text(tweet)):
        ...
        if has_sentiment(s):
            ss.append(sentiment_value(s))
    if ss:
        average = make_sentiment(sum(ss)/len(ss))
    else:
...
def find_centroid(polygon):
    ...
    pv = []
    for p in polygon:
        pv.append((latitude(p), longitude(p)))
    ...
    for p in pv:
        ...
        q = p
    if area == 0:
        return pv[0][0], pv[0][1], 0

    
    poly = Polygon(pv)
    
    p = poly.centroid
    return p.x, p.y, abs(area / 2)
...
def group_tweets_by_state(tweets):
    ...
    for tweet in tweets:
        center = tweet_location(tweet)
        dist_from_center = lambda name: geo_distance(center, us_centers[name])
        city = sorted(us_centers, key=dist_from_center)[0]
        if city in tweets_by_state:
            tweets_by_state[city].append(tweet)
        else:
            tweets_by_state[city] = [tweet]
    ...
    for state in tweets_by_state:
        ...
        if tweets:
            ...
            for tweet in tweets:
                ...
                if has_sentiment(s):
                    l.append(sentiment_value(s))
            if l:
                averaged_state_sentiments[state] = sum(l)/len(l)
